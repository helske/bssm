...
%\VignetteIndexEntry{bssm: Bayesian Inference of Non-linear and Non-Gaussian State Space Models in R}
  ...
  %\VignetteDepends{ggplot2, bayesplot, sde}
  ---
...
level <- expand_sample(mcmc_bsm, "alpha", states = 1)
...
require("ggplot2")
level_fit <- ts(colMeans(expand_sample(mcmc_bsm, "alpha")$level), start = start(model$y),
  frequency = 4)
...
```{Rcpp sde_ssm_template, code=readLines('sde_ssm_template.cpp'), eval = FALSE, echo = TRUE}
```