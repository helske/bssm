...
Note that most of this code is general and can be modified to specific models accordingly. Word of caution when using these `C++` snippets: Due to the use of pointers, users must recompile and construct the model objects in each `R` session, i.e. saving and later loading the model object is not sufficient. Also, repeatedly recompiling the external pointers from same file with `sourceCpp` can cause `R` crash unexpectedly if the corresponding cache directory is not changed between compilations^[1].
...
$$
with parameters $\theta = (\phi, \nu, \sigma) = (0.5, 2, 1)$ and the initial condition $\alpha_0 = 1$. For observation density, we use Poisson distribution with parameter $\exp(\alpha_k)$. We first simulate a trajectory $x_0, \ldots, x_n$ using the `sde.sim` function from the `sde` package [@sde] and use that for simulating the observations $y$:
```{r}
set.seed(1)
library("sde")
...
ng_model <- ng_ar1(y, rho = uniform(0.9, -1, 1), mu = normal(0, 0, 10),
  sigma = halfnormal(0, 10), distribution = "poisson")


out <- run_mcmc(model, n_iter = 6e4, n_burnin=1e4, nsim_state = 500, L_c = 1, L_f = 2, method = "da")
out2 <- run_mcmc(ng_model, n_iter = 6e4, nsim_states = 100)
...
While Kalman filter solves the filtering problem exactly in case of linear-Gaussian models, other previously mentioned approaches produce only approximate, possibly biased estimates.
linear-Gaussian models, `bssm` offers classical Kalman filtering and smoothing as functions `kfilter` and `smoother`. Simulation smoother [@durbin-koopman], as well as 
...
```{r mcmc}
mcmc_out <- run_mcmc(model, n_iter = 1e4)
mcmc_out
```
...
```{Rcpp nlg_ssm_template, code=readLines('nlg_ssm_template'), eval = FALSE, echo = TRUE}
```