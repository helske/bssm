...
```{r nhtemp_cpp}
Rcpp::sourceCpp("lgg_ssm_template.cpp", 
                cacheDir = paste0(tempdir(), "/", sample(1e8, 1))) # avoid recompilation issues
pntrs <- create_xptrs()
...
For models where the state equation is now longer linear-Gaussian, we can again use our pointer-based interface. General non-linear Gaussian model can be defined with the function `ssm_nlg`, with similar fashion as in case of `lgg_ssm`. A template for `ssm_nlg` can be found in the Appendix.
...
Rcpp::sourceCpp("ssm_sde_template.cpp", 
                cacheDir = paste0(tempdir(), "/", sample(1e8, 1)))
pntrs <- create_xptrs()
