...
```{r nhtemp_cpp}
library("Rcpp")
sourceCpp("lgg_ssm_template.cpp")
pntrs <- create_xptrs()
bsm_model2 <- lgg_ssm(y = nhtemp, Z = pntrs$Z, H = pntrs$H, T = pntrs$T, R = pntrs$R, 
  ...
  log_prior_pdf = pntrs$log_prior_pdf, theta = rep(1, 3), known_params = c(0, 1000), 
  n_states = 2, state_names = c("level", "slope"))
```
