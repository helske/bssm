...
In this vignette we will first introduce the basic state space modelling framework used in `bssm`, and the relevant algorithms. We then give several illustrations how to use `bssm` in practice.
...
Given the target acceptance rate $a^{\ast}$ (e.g. 0.234) and $\gamma \in (0,1]$, at iteration $i$:
...
In principle, when using the exact Bayesian inference we should simulate multiple realizations of the states $\alpha_{1:T}$ in each iteration of MCMC in order to compute $\log \frac{1}{N}\sum_{j=1}^N w_j^{\ast}$. Fortunately, we can use so called delayed acceptance (DA) approach [@Christen2005; @Banterle2015] which speeds up the computation considerably:
...
API of the `expand_sample` function is likely change in future. 
