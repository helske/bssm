...
```{r mcmc}
out_mcmc_pm <- run_mcmc(model, n_iter = 10000, nsim_states = 10, method = "da", 
  simulation_method = "psi")
out_mcmc_ekf <- run_mcmc(model, n_iter = 10000, method = "ekf")
...
Let's make some predictions based on our MCMC run. We first build a model for future observations, and then call `predict` method. Note that for EKF based prediction the forecasting is based on the summary statistics obtained with EKF, which produces smoother but potentially biased results.
```{r pred}
future_model <- model
future_model$y <- ts(rep(NA, 100), start = end(model$y) + c(1, 0))
out_pred_pm <- predict(out_mcmc_pm, future_model, type = "response")
out_pred_ekf <- predict(out_mcmc_ekf, future_model, type = "response")
library("ggplot2")
