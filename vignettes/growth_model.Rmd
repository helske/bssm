...
$$
with $t=1,\ldots, n$, $\epsilon_t ~ N(0,\textrm{I}_p)$, and $\eta ~ N(0,\textrm{I}_k)$. Here vector $\theta$ contains the unknown model parameters. 
...
r'_{t+1} = r'_t + \eta_t,
$$
with constant carrying capacity $K = 500$, initial population size $p_0 = 50$, initial growth rate on logit scale $r'_0 = -1.5$, $dt = 0.1$, $\xi \sim N(0,1)$, $\eta \sim N(0,0.05)$, and $\epsilon \sim N(0, 1)$.
...
library("dplyr")
library("Hmisc")
d1 <- as.data.frame(mcmc_res, "states")
d2 <- as.data.frame(mcmc_ekf_res, "states")
d1$method <- "is2-psi"
