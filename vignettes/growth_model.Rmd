...
```{r theta}
initial_theta <- c(1, 0.1, 0.1)

# dT, K, a1 and the prior variances
psi <- c(dT, 100, 0.3, 5, 10, 100);
```
...
For parameter inference, we can perform full Bayesian inference with \texttt{bssm}. There are multiple choices for the MCMC algorithm in the package, but currently the best option for models with non-linearities in state equation is boostrap-filter based pseudo-marginal MCMC.

```{r mcmc}
# out_bsf <- run_mcmc(model, n_iter = 1e4, nsim_states = 500, method = "pm", 
#   simulation_method = "bsf", delayed_acceptance = FALSE)
# out_bsf$acceptance_rate
# out_bsf$posterior
# 
# library(diagis)
