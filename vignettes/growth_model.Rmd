...
Instead of EKF-based analysis, we can perform full Bayesian inference on the Lotka-Volterra model. The MCMC algorithm is based on pseudo-marginal particle MCMC with bootstrap filter, but other algorithms are currently being implemented as well.

```{r mcmc}
out_bsf <- run_mcmc(model, n_iter = 1e4, nsim_states = 250, max_iter=0, method = "pm",
  simulation_method = "bsf",
  initial_mode = out_smoother, delayed_acceptance = FALSE)
out_bsf$acceptance
library(diagis)
ts.plot(cbind(y, p, weighted_mean(t(out_bsf$alpha[,2,]), out_bsf$counts)), col = 1:3)
...
This is the full `growt_model_functions.cpp` file:
