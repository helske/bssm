...
out_psi <- run_mcmc(model, n_iter = 2e4, nsim_states = 10, max_iter=0, method = "pm",
...
out_pred <- predict(out_psi, future_model)
ts.plot(model$y, ts(cbind(rowMeans(out_pred[1,,]), 
  t(apply(out_pred[1,,], 1, quantile, prob = c(0.05, 0.95)))), 
  start = end(model$y)), col = c(1, 2, 3,3))
```
## Appendix
