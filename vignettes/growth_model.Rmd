...
For parameter inference, we can perform full Bayesian inference with \texttt{bssm}. There are multiple choices for the MCMC algorithm in the package, and here we will use $\psi$-PF based pseudo-marginal MCMC with delayed acceptance.

```{r mcmc}
out_mcmc <- run_mcmc(model, n_iter = 2e4, nsim_states = 10, method = "pm", 
  simulation_method = "psi")
out_mcmc$acceptance_rate
out_mcmc2 <- run_mcmc(model, n_iter = 2e4, nsim_states = 250, method = "isc", 
  simulation_method = "bsf")
out_mcmc2$acceptance_rate
out_mcmc2b <- run_mcmc(model, n_iter = 2e4, nsim_states = 750, method = "isc", 
  simulation_method = "bsf")
out_mcmc2b$acceptance_rate
# 
# library(diagis)
# p_bsf <- weighted_mean(t(out_bsf$alpha[,2,]), out_psi$counts)

#ts.plot(cbind(y, p, p_bsf), col = 1:3)

out_ekf <- run_mcmc(model, n_iter = 1e4, nsim_states = 1, method = "ekf",
  delayed_acceptance = FALSE)
out_ekf$acceptance
p_ekf <- weighted_mean(t(out_ekf$alpha[,2,]), out_ekf$counts)
ts.plot(cbind(y, p, p_ekf), col = 1:3)
#ts.plot(cbind(p_bsf, p_ekf), col = 1:3)
```
...
out_pred <- predict(out_ekf, future_model, type = "mean")
library("ggplot2")
