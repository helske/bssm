...
test_that("MCMC results for Poisson model are correct",{
  ...
  expect_error(mcmc_poisson <- run_mcmc(model_bssm, n_iter = 100, nsim = 5, seed = 42), NA)
  
  expect_equal(run_mcmc(model_bssm, n_iter = 100, seed = 1, nsim = 5)[-14], 
    run_mcmc(model_bssm, n_iter = 100, seed = 1, nsim = 5)[-14])
  expect_equal(run_mcmc(model_bssm, n_iter = 100, seed = 1, type = "summary", nsim = 5)[-15], 
    run_mcmc(model_bssm, n_iter = 100, seed = 1, type = "summary", nsim = 5)[-15])
  expect_equal(run_mcmc(model_bssm, n_iter = 100, seed = 1, type = "theta", nsim = 5)[-13], 
    run_mcmc(model_bssm, n_iter = 100, seed = 1, type = "theta", nsim = 5)[-13])
...
test_that("MCMC results for SV model using IS-correction are correct",{
  ...
  expect_equal(run_mcmc(model_bssm, n_iter = 100, nsim = 10,
    method = "is2", seed = 1)[-15], 
    run_mcmc(model_bssm, n_iter = 100, nsim = 10, method = "is2", seed = 1)[-15])
  
  expect_equal(run_mcmc(model_bssm, n_iter = 100, nsim = 10,
    method = "is2", seed = 1, simulation_method = "psi")[-15], 
    run_mcmc(model_bssm, n_iter = 100, nsim = 10, 
      method = "is2", seed = 1, simulation_method = "psi")[-15])
  ...
  expect_error(mcmc_sv <- run_mcmc(model_bssm, n_iter = 100, nsim = 10,
