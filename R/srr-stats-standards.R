...
#'
#' Explicit conversions are used where necessary
...
#' 
#' Only matrix/mts/arrays are supported, not data.frame style objects.
...
#' 
#' Correctness is demonstrated in Helske & Vihola (2021) (comparison with Stan) and Vihola & Helske & Franks (2020) (correcness of the IS-MCMC etc)
#' Full correctness tests cannot be performed without overly extensive time requirements due to Monte Carlo variation.
...
#' 
#' Tested by autotest and testthat tests
...
#' 
#' Tested by autotest and package documentation
...
#' 
#' Handled by the prior construction functions
...
#' 
#' Checked, explained and done in run_mcmc
...
#' 
#' As with the the general standards.
...
#' 
#' run_mcmc returns some of these, other are embedded in the input model itself (starting values)
...
#' 
#' Demonstrated in the vignettes, examples, papers and tests
...
#' 
#' Demonstrated in test_mcmc.R. 
#' The scales do not matter (in terms of runtime) in random walk Metropolis nor in particle filters, as long as numerical issues are not encountered
#' @srrstats {BS7.3} *Bayesian software should include tests which demonstrate and confirm the scaling of algorithmic efficiency with sizes of input data.* 
#' @srrstats {BS7.4a} *The implications of any assumptions on scales on input objects should be explicitly tested in this context; for example that the scales of inputs which do not have means of zero will not be able to be recovered.*
#' 
#' @noRd
...
#'
#' No data.frame style tabular data is used/supported as input
...
#' 
#' No output is written to local files
...
#' 
#' Package does not contain extended tests (although benchmarks folder contains template for running such very time-consuming tests)
...
#' @srrstatsNA {BS4.5} *Ensure that appropriate mechanisms are provided for models which do not converge.* 
...
#' 
#' Of course applicable, but at the moment this is not done (issue has been open several years in github...).
#' @srrstatsNA {BS2.12} *Bayesian Software should implement at least one parameter controlling the verbosity of output, defaulting to verbose output of all appropriate messages, warnings, errors, and progress indicators.*
#' @srrstatsNA {BS2.13} *Bayesian Software should enable suppression of messages and progress indicators, while retaining verbosity of warnings and errors. This should be tested.*
#' @srrstatsNA {BS2.14} *Bayesian Software should enable suppression of warnings where appropriate. This should be tested.*
#' @srrstatsNA {BS2.15} *Bayesian Software should explicitly enable errors to be caught, and appropriately processed either through conversion to warnings, or otherwise captured in return values. This should be tested.* 
#' 
#' Not really relevant for SSMs, or at least difficult to check this kind of thing in general
#' @srrstatsNA {BS3.1} *Implement pre-processing routines to diagnose perfect collinearity, and provide appropriate diagnostic messages or warnings*
#' @srrstatsNA {BS3.2} *Provide distinct routines for processing perfectly collinear data, potentially bypassing sampling algorithms* 
#'
#' Just suggests using ggplot or bayesplot packages, with several examples 
#' @srrstatsNA {BS6.1} *Software should implement a default `plot` method for return objects*
#' @srrstatsNA {BS6.2} *Software should provide and document straightforward abilities to plot sequences of posterior samples, with burn-in periods clearly distinguished*
#' @srrstatsNA {BS6.3} *Software should provide and document straightforward abilities to plot posterior distributional estimates* 
#' @srrstatsNA {BS6.5} *Software may provide abilities to plot both sequences of posterior samples and distributional estimates together in single graphic* 
#'
#' The computation of posterior predicted/fitted value is somewhat time consuming so it is not done automatically (there is a fitted/predict functions for that)
