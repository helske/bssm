...
autoplot.predict_bssm <- function(object, plot_mean = TRUE,
  ...
  if (!missing(y) && !is.ts(y)) {
    y <- ts(y, start = start(object$mean), frequency = frequency(object$mean))
  }
  
  plot_y <- !missing(y)
  if (!missing(fit) && !is.ts(fit)) {
    fit <- ts(fit, start = start(object$mean), frequency = frequency(object$mean))
  }
  plot_fit <- !missing(fit)
  d <- ts.union(if (plot_y) y, if (plot_mean) object$mean, object$intervals, 
    if (plot_fit) fit)
  ...
  names(d) <-  c("time", if (plot_y) "y", if (plot_mean) "mean",
  ...
  intv <- names(d)[-(1:(plot_y + 1 + plot_mean))]
