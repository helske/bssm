...
#' @param expand Should the jump-chain be expanded? Defaults to \code{TRUE} for non-IS-MCMC, and \code{FALSE} for IS-MCMC. 
#' For \code{expand = FALSE} and always for IS-MCMC, the resulting data.frame contains variable weight (= counts times IS-weights).
#' @param ... Ignored.
#' @export
as.data.frame.mcmc_output <- function(x, 
  ...
  if (variable == "theta") {
    if (expand) {
      ...
      iters <- seq(x$n_burnin + 1, x$n_iter, by = x$n_thin)
      weights <- if(x$mcmc_type %in% paste0("is", 1:3)) rep(x$weights, times = x$counts) else 1
    } else {
      values <- x$theta
      iters <- x$n_burnin + cumsum(x$counts)
